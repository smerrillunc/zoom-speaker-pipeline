...
Four scripts in `pipeline/`. Everything in `pipeline/helpers/` is code they call.

| Script | Input → output | Runs on |
|---|---|---|
| `ocr.py` | video → `<id>.ocr.json`: raw on-screen names, one read per second | CPU, ~1 s per frame on 4 cores |
| `transcribe.py` | video → `<id>.asr.json`: Whisper segments + pyannote voice clusters | GPU, ~22× real time (large-v2, L40) |
| `clean_ocr.py` | a collection's `*.ocr.json` → `<id>.speakers.json` per meeting + `registry.json` (who is who) | CPU, seconds per meeting |
| `merge.py` | `<id>.asr.json` + `<id>.speakers.json` → `<id>.json`, the speaker-attributed transcript | CPU, seconds per meeting |

## Running order

```
                ┌─ ocr.py ─────────► *.ocr.json ─► clean_ocr.py ─► *.speakers.json ─┐
 meeting videos ┤                                                                    ├─► merge.py ─► transcripts
                └─ transcribe.py ──► *.asr.json ─────────────────────────────────────┘
```

1. **`ocr.py` and `transcribe.py`, in either order or at the same time.** Both read
   the video and neither needs the other's output. Run them as separate jobs (OCR on
   CPU nodes, transcription on GPU nodes), not in one Python process: PaddleOCR and
   CTranslate2 load conflicting OpenMP runtimes.
2. **`clean_ocr.py`, once every meeting of a collection has its `*.ocr.json`.** It
   links the same person across meetings, so it needs the whole collection (all of
   one council's or one court's meetings) in one run. Adding meetings later means
   running it again over the full collection.
3. **`merge.py`, last.** It needs each meeting's `*.asr.json` from `transcribe.py`
   and its `*.speakers.json` from `clean_ocr.py`. Meetings missing either are
   skipped with a message.

`clean_ocr.py` and `merge.py` need only `rapidfuzz` and take minutes for thousands of
meetings. The expensive `ocr.py` and `transcribe.py` outputs can be re-cleaned and
re-merged anywhere, for example after changing a threshold.

## Layout

```
pipeline/
    ocr.py              video → raw on-screen names
    transcribe.py       video → Whisper segments + voice clusters
    clean_ocr.py        raw names → people, per collection
    merge.py            transcript + people → speaker-attributed transcript
    helpers/            code the scripts call; nothing here is run directly
...
run_pipeline.sh         all four scripts, in order, over one folder of videos
...
export HF_TOKEN=hf_...                     # for pyannote voice clusters (transcribe.py)

python pipeline/ocr.py        meetings/ --out work/
python pipeline/transcribe.py meetings/ --out work/
python pipeline/clean_ocr.py  work/ --out work/ --context "Riverton City Council, Oregon"
python pipeline/merge.py --asr work/ --speakers work/ --out transcripts/ --formats json,txt
```

Or all four in order: `./run_pipeline.sh meetings/ work/ "Riverton City Council, Oregon"`.

Every script skips meetings that already have output (`--overwrite` redoes them) and
writes through a temporary file, so an interrupted run never leaves half a result.
`ocr.py` and `transcribe.py` accept `--shard i/n`, so a job array can split a folder
across tasks.
...
After `ocr.py`, check each meeting's **grid fraction** (`frames_grid / frames_sampled`
in `<id>.ocr.json`, also in `<id>.stats.json` after `merge.py`):
...
### `ocr.py`: who is on screen

```bash
python pipeline/ocr.py VIDEO_OR_DIR... --out DIR [--sampling-rate 1.0] [--no-super-resolution] [--shard i/n]
```
...
### `transcribe.py`: what was said, and in which voice

```bash
python pipeline/transcribe.py VIDEO_OR_DIR... --out DIR [--whisper large-v2] [--clustering auto|on|off] [--shard i/n]
```
...
token, segments are left unclustered and `merge.py` names each one from screen time alone.

### `clean_ocr.py`: raw reads → people

```bash
python pipeline/clean_ocr.py DIR --out DIR [--context "Institution, Place"] [--no-cross-meeting]
```
...
### `merge.py`: attribute the text

```bash
python pipeline/merge.py --asr DIR --speakers DIR --out DIR [--formats json,txt,srt,vtt] [--lag 0.6]
```
...
|---|---|---|
| `paddlex[ocr]` | `3.2.1` with the `[ocr]` extra | every frame reads as no name (`ocr.py`'s preflight catches this) |
...
| Variable | Purpose |
|---|---|
| `HF_TOKEN` | access to the gated pyannote model (`transcribe.py`) |
...
- `clean_ocr.py` and `merge.py` are deterministic: the same inputs give byte-identical
  outputs, whatever the input order. `transcribe.py` always uses faster-whisper, never an optional
